...
# Want to simulate 100 observations with randomizing date, beach
# and E. Coli level

# Before simulating, we must give the range of the dates in the
# original data set
...
simulated_beach_water_data <-
  tibble(
    ...
    "Collection Date" = sample(x = date_range, size = 100, replace = TRUE),

    # Randomly picking between the 2 beaches in the data set 1000 times
    "Beach Name" = sample(x = c("Marie Curtis Park East Beach",
                                "Sunnyside Beach"), size = 100, replace = TRUE),

    # Randomly simulating an E. Coli level, 1000 times
    "E. Coli Level (# colonies/100mL)" = sample(x = 0:6191768, size = 100,
                                                replace = TRUE))
...
simulated_beach_water_data$`E. Coli Level (# colonies/100mL)` >=
  min_ecoli_level & simulated_beach_water_data$
  `E. Coli Level (# colonies/100mL)` <= max_ecoli_level
