...
# read file first
raw_beach_water_data <- read_csv("toronto-beaches-water/input/data/
                                 raw_beach_water_data.csv")

# select only rows of interest
beach_water_cleaned_data <- raw_beach_water_data |>
  select(X_id, beachId, beachName, siteName, collectionDate, eColi, comments)
...
beach_water_cleaned_data <- beach_water_cleaned_data |> filter(comments !=
                                                                 "No data")
